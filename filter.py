...
def Hessian(img):
    ...
    img = cv2.ximgproc.RidgeDetectionFilter_create().getRidgeFilteredImage(img)
    return img
...
def Laplacian(img):
    cv2.Laplacian(img, ddepth=ddepth, ksize=5)
    return img
...
def Gabor(img):
    ...
    img = cv2.filter2D(img, ddepth=ddepth, kernel=kernel)
    return img


def Sobel(img):
    '''https://docs.opencv.org/2.4/doc/tutorials/imgproc/imgtrans/sobel_derivatives/sobel_derivatives.html'''
    img = cv2.Sobel(img, ddepth=ddepth, dx=1, dy=1)
    return img

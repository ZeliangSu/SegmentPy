...
def model_improved(train_inputs, test_inputs, patch_size, batch_size, conv_size, nb_conv, learning_rate=0.0001):
    ...
    with tf.name_scope('model'):
        ...
        with tf.name_scope('encoder'):
            ...
            conv3, m3 = conv2d_layer(conv2_pooling, shape=[conv_size, conv_size, nb_conv * 2, nb_conv * 2], name='conv3')
            conv3bis, m3b = conv2d_layer(conv3, shape=[conv_size, conv_size, nb_conv * 2, nb_conv * 2], name='conv3bis')
            conv3bisbis, m3bb = conv2d_layer(conv3, shape=[conv_size, conv_size, nb_conv * 2, nb_conv * 2], name='conv3bis')

        with tf.name_scope('dnn'):
            conv4_flat = reshape(conv3bisbis, [-1, patch_size ** 2 // 16], name='flatten')
            full_layer_1, mf1 = normal_full_layer(conv4_flat, patch_size ** 2 // 64, name='dnn1')
            full_dropout1 = dropout(full_layer_1, drop_prob, name='dropout1')
            full_layer_2, mf2 = normal_full_layer(full_dropout1, patch_size ** 2 // 128, name='dnn2')
            full_dropout2 = dropout(full_layer_2, drop_prob, name='dropout2')
            full_layer_3, mf3 = normal_full_layer(full_dropout2, patch_size ** 2 // 64, name='dnn3')
        ...
        with tf.name_scope('decoder'):
            deconv_4, m4 = conv2d_transpose_layer(dnn_reshape, [conv_size, conv_size, 1, nb_conv * 4], X_dyn_batsize, name='deconv4')  #[height, width, in_channels, output_channels]
            deconv_4bis, m4b = conv2d_transpose_layer(deconv_4, [conv_size, conv_size, nb_conv * 4, nb_conv * 8], X_dyn_batsize, stride=1, name='deconv4bis')  #fixme: strides should be 2
            concat1, outshape4 = concat([up_2by2(deconv_4bis, name='up1'), conv3], name='concat1')

            deconv_5, m5 = conv2d_transpose_layer(concat1, [conv_size, conv_size, int(outshape4[3]), nb_conv * 2], X_dyn_batsize, name='deconv5')
            deconv_5bis, m5b = conv2d_transpose_layer(deconv_5, [conv_size, conv_size, nb_conv * 2, nb_conv * 2], X_dyn_batsize, stride=1, name='deconv5bis')  #fixme: strides should be 2
            concat2, outshape5 = concat([up_2by2(deconv_5bis, name='up2'), conv2], name='concat2')

            deconv_6, m6 = conv2d_transpose_layer(concat2, [conv_size, conv_size, int(outshape5[3]), nb_conv * 2], X_dyn_batsize, name='deconv6')
            deconv_6bis, m6b = conv2d_transpose_layer(deconv_6, [conv_size, conv_size, nb_conv * 2, nb_conv * 2], X_dyn_batsize, stride=1, name='deconv6bis')  #fixme: strides should be 2
            concat3, outshape6 = concat([up_2by2(deconv_6bis, name='up3'), conv1], name='concat3')

            deconv_7, m7 = conv2d_transpose_layer(concat3, [conv_size, conv_size, int(outshape6[3]), nb_conv], X_dyn_batsize, name='deconv7')
            deconv_7bis, m7b = conv2d_transpose_layer(deconv_7, [conv_size, conv_size, nb_conv, nb_conv], X_dyn_batsize, name='deconv7bis')
            logits, m7bb = conv2d_transpose_layer(deconv_7bis, [conv_size, conv_size, nb_conv, 1], X_dyn_batsize, name='deconv7bisbis')  #fixme: change activation function. 0 everywhere prediction?
    ...
    with tf.name_scope('summary'):
        merged = tf.summary.merge([m1, m1b, m2, m2b, m3, m3b, m4, m4b, mf1, mf2, mf3,
                                   m5, m5b, m6, m6b, m7, m7b, m_loss, m_acc, grad_sum])  #fixme: withdraw summary of histories for GPU resource reason
    return {
        'y_pred': logits,
        'train_op': train_op,
        'drop': drop_prob,

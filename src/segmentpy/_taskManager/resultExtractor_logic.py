...
class rltExtractor_logic(QWidget, Ui_Extractor):
    ...
    def makeLegend(self):
        ...
        data = data.reshape(
            tuple(map(add, fig2.canvas.get_width_height()[::-1], fig2.canvas.get_width_height()[::-1])) + (3,))
        logger.debug('grid shape: {}'.format(data.shape))
    ...
    def saveResult(self):
        ...
        self.fig2.savefig(os.path.join(save_path, 'legend.png'), bbox_inches='tight', pad_inches=0)
